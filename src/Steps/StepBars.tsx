...
const StepBars = <InputDate, ParsedDate, InputDuration, Units>() => {
  ...
  return (
    <div className={cx('stepBars')}>
      {timelineContext && timelineContext.stepLevels.flatMap((level, levelIndex) => {
        return level.steps.map((step, stepIndex) => {
          return step.offset > 0 && step.offset <= 100 && (
            <div
              key={`stepBar-${levelIndex}-${stepIndex}`}
              className={cx('stepBar')}
              style={{
                left: step.offset + '%',
                borderColor: getColor(levelIndex, timelineContext.calendar.isImportantStep(step)),
              }}
            />
          );
        });
