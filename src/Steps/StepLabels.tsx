...
const StepLabels = <InputDate, ParsedDate, InputDuration, Units>(props: Props) => {
  ...
  const getColor = (stepLevel: number, isImportant: boolean) => {
    if (stepLevel === 0) {
      return isImportant ? themeContext.secondaryColor : themeContext.primaryColor;
    } else if (stepLevel === 1) {
      return isImportant ? themeContext.tertiaryColor : themeContext.secondaryColor;
    } else {
  ...
  return (
    <div className={cx('stepLabels')}>
      <StepBars />
      {
        timelineContext && timelineContext.stepLevels.map((stepLevel, stepLevelIndex) => (
          <div
            key={`stepLevel-${stepLevelIndex}`}
            className={cx(
              stepLevelIndex === 0 ? 'stepLabelsPrimary' : 'stepLabelsSecondary',
                ...
                return (
                  <div
                    ...
                    className={cx(
                      ...
                      timelineContext.calendar.isImportantStep(step) ? 'stepLabelImportant' : ''
                    )}
                    style={{
                      left: left + '%',
                      width: width + '%',
                      color: getColor(stepLevelIndex, timelineContext.calendar.isImportantStep(step)),
                    }}
                  >
                    <div>{timelineContext.calendar.format(step.date, step.unit, stepLevelIndex === 0)}</div>
                  </div>
                );
              })
            }
